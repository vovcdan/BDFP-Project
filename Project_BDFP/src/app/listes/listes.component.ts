...
export class DialogOverviewExampleDialog implements OnInit {
  ...
  ajoutListe() {
    this.onDisable = true;
      for(let i = 0; i < this.testEqual.length; i++) {
        if(this.testEqual[i].titrelist == this.data.title || this.data.title == "") {
          return;
        }
      }
      this.filmService.addListToAllLists(this.utilService.getUserId(), this.data.title).subscribe((log) => {
